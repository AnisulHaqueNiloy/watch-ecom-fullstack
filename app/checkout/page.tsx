...
export default function CheckoutPage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const whatsappPhone = process.env.NEXT_PUBLIC_WHATSAPP_NUMBER || "+8801631484734"
  ...
  const generateWhatsAppMessage = (user: any, formData: any, items: any[], total: string) => {
    ...
    <div className="min-h-screen bg-background">
      ...
      <nav className="bg-card border-b border-border sticky top-0 z-40">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <Link href="/shop" className="font-serif text-2xl font-bold text-primary">
            TIMEKEEPER
          </Link>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="mb-12 animate-slide-down">
          <h1 className="font-serif text-5xl font-bold mb-2">Secure Checkout</h1>
          <p className="text-muted-foreground">Complete your purchase securely</p>
        </div>

        <div className="grid md:grid-cols-3 gap-8">
          <div className="md:col-span-2 animate-slide-left">
            <form onSubmit={handleSubmit} className="bg-card border-2 border-primary/20 p-8 rounded-lg space-y-6">
              <h2 className="font-serif text-2xl font-bold mb-8 text-foreground">Delivery Information</h2>
              ...
              <p className="text-xs text-muted-foreground text-center pt-4">
                By placing this order, you agree to our terms and conditions. You will be redirected to WhatsApp to
                confirm with our admin team.
              </p>
            </form>
          </div>
            ...
            <div className="bg-card border-2 border-primary/20 p-8 rounded-lg sticky top-24 space-y-6">
              ...
              <div className="space-y-4 pb-6 border-b-2 border-border">
                {items.map((item, idx) => (
                  <div key={idx} className="flex justify-between items-start">
                    ...
                    <p className="font-semibold text-accent ml-4">${(item.price * item.quantity).toFixed(2)}</p>
                  </div>
                ))}
              </div>
              ...
              <div className="space-y-3">
                ...
                <div className="flex justify-between border-t-2 border-border pt-3">
                  ...
                  <span className="font-serif text-2xl font-bold text-accent">${calculateTotal()}</span>
                </div>
              </div>

              <div className="bg-primary/10 border border-primary/20 p-4 rounded-lg text-sm">
                <p className="text-foreground">
                  <span className="font-semibold">Secure Payment:</span> WhatsApp-based order confirmation
                </p>
              </div>
            </div>
          </div>
